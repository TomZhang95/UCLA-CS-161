...
(defun at-least-one-color (n c k)
  (cond
    ;Condistions to stop the recursion
    ((< k c) nil)

    ;Get and append every node index before the end of color index to a list
...
(defun at-most-one-color (n c k)
  (cond
    ((< k c) nil)
    ;Iterate through all the posible pairs
    (t (append (clauses-of-only-one-in-this-pair n c c k) (at-most-one-color n (+ c 1) k)))
    )
  )

;return a list of clauses such as: [[a, b] [-c, d] ...] which means
;if node n choose a, then it cannot choose b any more, and if node n
;choose -c, then it cannot choose d anymore.
(defun clauses-of-only-one-in-this-pair (n c1 c2 k)
  (cond
    ((< k c1) nil)
    ((< k c2) nil)

    ;if c1 = c2, increase c2 by 1 and continue the iteration
    ((= c1 c2) (clauses-of-only-one-in-this-pair n c1 (+ c2 1) k))

    ;otherwise, node n cannot get color1 and color2 at the same time
    (t (append 
      ;Makes these two colar a clause, and iterative append the list of clauses
      (list (list (- 0 (node2var n c1 k)) (- 0 (node2var n c2 k))))
      (clauses-of-only-one-in-this-pair n c1 (+ c2 1) k)
      ))
    )
  )
; EXERCISE: Fill this function
...
(defun generate-edge-clauses (e k)
  (generate-edge-clauses-wrapper e 1 k)
  )

(defun generate-edge-clauses-wrapper (e c k)
  (cond ((< k c) NIL)
        (t (append (list (different-colors-clauses e c k)) (generate-edge-clauses-wrapper e (+ c 1) k)))
        )
  )

(defun different-colors-clauses (e c k)
  (list (- 0 (node2var (first e) c k)) (- 0 (node2var (second e) c k)))
    )
